...
public class MapperAvisoEditarImplementacao implements MapperCatchupInterface<AvisoEditarDto,Aviso >{




	@Override
	public Aviso dtoParaEntidade(AvisoEditarDto element) {
	...
	@Override
	public AvisoEditarDto entidadeParaDto(Aviso element) {
		ModelMapper modelMapper = new ModelMapper();
		modelMapper.getConfiguration().setMatchingStrategy(MatchingStrategies.LOOSE)
				.setFieldAccessLevel(AccessLevel.PRIVATE).setFieldMatchingEnabled(true);

		return modelMapper.map(element, AvisoEditarDto.class);
	}



	@Override
	public List<AvisoEditarDto> listaEntidadeParaDto(List<Aviso> element) {
